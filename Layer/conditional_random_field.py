...
class ConditionalRandomField(torch.nn.Module):
    ...
    def viterbi_tags(self,
        ...
        start_tag = num_tags
        end_tag = num_tags + 1
        transitions = Variable(torch.Tensor(num_tags + 2, num_tags + 2).fill_(-10000.))
        if USE_CUDA:
        ...
        if self.include_start_end_transitions:
            #print("self.start_transitions", self.start_transitions)
            #print("self._constraint_mask", self._constraint_mask)

            transitions[start_tag, :num_tags] = (
                    self.start_transitions.detach().data * self._constraint_mask[start_tag, :num_tags].data + 
                    -10000.0 * (1 - self._constraint_mask[start_tag, :num_tags].detach().data)
            )
            transitions[:num_tags, end_tag] = (
                    self.end_transitions.detach().data * self._constraint_mask[:num_tags, end_tag].data +
                    -10000.0 * (1 - self._constraint_mask[:num_tags, end_tag].detach().data)
            )
        else:
        ...
        tag_sequence = Variable(torch.Tensor(max_seq_length + 2, num_tags + 2))
        if USE_CUDA:
            tag_sequence = tag_sequence.cuda()
...
def test():
    k = is_transition_allowed(constraint_type="BIO",
                          from_tag="START",
                          from_entity=None,
                          to_tag="END",
                          to_entity=None
